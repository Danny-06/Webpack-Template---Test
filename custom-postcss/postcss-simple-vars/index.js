      ...
      return {
        ...
        AtRule (node, helpers) {
          switch (node.name) {
            ...
            case 'each':
              const expression = ' in '
              const eachCollectionIndex = node.params.indexOf(expression)
              const eachCollection = node.params.slice(eachCollectionIndex + expression.length)

              if (eachCollection.startsWith('$')) {
                node.params = node.params.replace(eachCollection, variables[eachCollection.slice(1)])
              }
            break
